...
#include "fam_1bit_1ras_64k.h"
#include "fam_1bit_1ras_64k.pio.h"
...
static void setup_pio(const uint8_t *delay_set)
{
    ...
    bool rc = pio_claim_free_sm_and_add_program_for_gpio_range(
        get_patched_program(
            &fam_1bit_1ras_64k_program, delay_set, FAM_1BIT_1RAS_64K_DELAY_SET_COLS
        ),
    ...
    pio_sm_config c = fam_1bit_1ras_64k_program_get_default_config(offset);
...
static void teardown_pio()
{
    ...
    pio_remove_program_and_unclaim_sm(&fam_1bit_1ras_64k_program, pio, sm, offset);
}
