...
#include "fam_1bit_1ras_256k.h"
#include "fam_1bit_1ras_256k.pio.h"
#include "logging.h"
...
static void setup_pio(const delay_set_t delay_set)
{
    ...
    bool rc = pio_claim_free_sm_and_add_program_for_gpio_range(
        get_patched_program(
            &fam_1bit_1ras_256k_program, delay_set, FAM_1BIT_1RAS_256K_DELAY_SET_COLS
        ),
    ...
    pio_sm_config c = fam_1bit_1ras_256k_program_get_default_config(offset);
...
static void teardown_pio()
{
    ...
    pio_remove_program_and_unclaim_sm(&fam_1bit_1ras_256k_program, pio, sm, offset);
}
...
const mem_family_t *fam_1bit_1ras_256k() {
    return &self;
}